...
outdirect = 'E:\BMVC2019\HMDB_bmvc_flow_combie_lrsr\';
if ~exist(outdirect)
    mkdir(outdirect)
end
viddirect = 'E:\BMVC2019\HMDB_bmvc_vid_combine_lrsr\';
src = dir(viddirect);
src = src(3:end);
count = 0;
...
for k = 1:length(src)
    ...
    for l = 1:length(fdir)
        ...
        system(['denseFlow.exe -f ',namee,' -x ',folderfilex,'\flow_x -y ',folderfiley,'\flow_y -b 20 -t 1 -d 3']);
        count = count+1;
